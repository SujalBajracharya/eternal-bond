...
const getDetails = (dto: any): DetailItem[] => {
  ...
  if (dto.religion) details.push({ icon: Landmark, label: "Religion", value: dto.religion });
  if (dto.motherTongue) details.push({ icon: Languages, label: "Mother tongue", value: dto.motherTongue });
  if (dto.maritalStatus) details.push({ icon: BadgeCheck, label: "Marital status", value: dto.maritalStatus });
  if (dto.incomeRange) details.push({ icon: Wallet, label: "Income range", value: dto.incomeRange });
  return details;
};
...
const getFamilyDetails = (dto: any) => {
  ...
  if (dto.fatherOccupation) parts.push(`Father's Occupation: ${dto.fatherOccupation}`);
  if (dto.motherOccupation) parts.push(`Mother's Occupation: ${dto.motherOccupation}`);
    ...
    const fetchDailyMatches = async () => {
      ...
      try {
        ...
        const mapped: Match[] = data.map((dto: any) => {
          const mainPhoto = (dto.photos && dto.photos.length > 0) ? dto.photos[0] : (dto.avatarUrl || "");
          ...
          return {
            ...
            mood: dto.profession ? `${dto.profession} based in ${dto.location || "India"}` : "Looking for a lifetime connection",
            photo: mainPhoto,
    ...
    <main className="min-h-screen bg-background relative overflow-x-hidden">
      {loading ? (
        <div className="min-h-[80vh] flex flex-col justify-center items-center">
          ...
          <p className="text-muted-foreground mt-4 font-serif text-lg">Finding today's matches for you...</p>
        </div>
      ) : (
        <>
                  ...
                  {expanded && (
                    <div className="mt-4 space-y-5 animate-fade-in">
                      ...
                      {current.details.length > 0 && (
                        <div>
                          <div className="text-xs uppercase tracking-wider text-muted-foreground mb-2">Good to know</div>
                            ...
                            {current.details.map((d, i) => {
                              ...
                              return (
                                <div key={i} className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm">
                                  ...
                                  <span className="text-foreground/85 font-medium text-right">{d.value}</span>
                                </div>
                              );
                            })}
                          </div>
