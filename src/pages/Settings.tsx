...
import {
  ArrowLeft, User, Eye, Lock, Phone, Users2, Bell, Sparkles, ShieldCheck,
  ChevronRight, Check, Music2, Play, Pause, BadgeCheck, Heart, Palette,
  MessageCircle, Crown, Plus, Mail, Smile,
} from "lucide-react";
...
type SectionId =
  | "profile" | "photos" | "contact" | "family"
  | "notifications" | "personalization" | "safety";

const SECTIONS: { id: SectionId; label: string; icon: any; hint: string }[] = [
  { id: "profile", label: "Profile & Account", icon: User, hint: "Your basics" },
  { id: "photos", label: "Photo Privacy", icon: Eye, hint: "Who sees what" },
  { id: "contact", label: "Contact Preferences", icon: Phone, hint: "How to be reached" },
  { id: "family", label: "Family Involvement", icon: Users2, hint: "Guardians & roles" },
  { id: "notifications", label: "Notifications", icon: Bell, hint: "Quiet by default" },
  { id: "personalization", label: "Personalization", icon: Sparkles, hint: "Make it yours" },
  { id: "safety", label: "Safety & Verification", icon: ShieldCheck, hint: "Trust signals" },
];
...
const SectionHeader = ({
...
}: { icon: any; eyebrow: string; title: string; description: string }) => (
  <div className="flex items-start gap-4 p-7 pb-5">
    ...
    <div className="min-w-0">
      <div className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground/80">{eyebrow}</div>
...
const Row = ({
  title, hint, children, onClick,
}: { title: string; hint?: string; children?: React.ReactNode; onClick?: () => void }) => (
  <div
    onClick={onClick}
    className={cn(
...
const PhotoPrivacySection = () => {
  const [vis, setVis] = useState<("visible" | "blurred")[]>(["visible", "visible", "blurred", "blurred"]);
      ...
      <div className="px-7 pb-7 space-y-3">
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium">Photo {i + 1}{i === 0 && <span className="ml-2 text-[10px] uppercase tracking-wider text-primary">Main</span>}</div>
                <div className="text-xs text-muted-foreground mt-0.5">
                  {v === "visible" ? "Anyone viewing your profile can see this." : "Softly blurred until you both connect."}
                </div>
              </div>
              <div className="flex items-center gap-1 rounded-full bg-secondary/60 p-1 shrink-0">
                <button
                  ...
                  className={cn(
                    ...
                    v === "visible" ? "bg-card shadow-soft" : "text-muted-foreground hover:text-foreground",
                  )}
                >
                ...
                <button
                  ...
                  className={cn(
                    ...
                    v === "blurred" ? "bg-card shadow-soft" : "text-muted-foreground hover:text-foreground",
                  )}
                >
        ...
        <div className="rounded-2xl border border-border/60 bg-gradient-blush p-5 mt-4">
          ...
          <div className="grid grid-cols-4 gap-2.5">
            {PHOTOS.map((url, i) => (
              <div key={`p-${url}`} className="relative aspect-square rounded-xl overflow-hidden bg-secondary/40">
                <img
                  src={url}
                  alt=""
                  className={cn("w-full h-full object-cover transition-all duration-700", vis[i] === "blurred" && "blur-xl scale-125")}
                />
                {vis[i] === "blurred" && (
...
const ContactSection = () => {
  ...
  const opts = [
    { id: "user", label: "User-only contact", hint: "Only you receive messages and interest notifications.", icon: User },
    { id: "family", label: "Family-first contact", hint: "Your designated family members are notified before you, ensuring matches are reviewed together.", icon: Users2 },
    { id: "hybrid", label: "Hybrid contact", hint: "Both you and your family receive messages — a balanced, transparent approach.", icon: Heart },
  ] as const;
  return (
    <Surface>
      <SectionHeader
          ...
          return (
            ...
            >
              <div className={cn(
                ...
                active ? "bg-gradient-sunset text-white" : "bg-secondary/60 text-foreground",
              )}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {active && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-primary text-primary-foreground font-medium">Selected</span>
                  )}
                </div>
                <p className="text-xs text-muted-foreground mt-1 leading-relaxed">{o.hint}</p>
              </div>
              <div className={cn(
...
const FamilySection = () => {
  ...
  return (
    <Surface>
      ...
      <div className="px-7 pb-7 space-y-3">
        {family.map((f) => (
          <div key={f.name} className="flex items-center gap-4 rounded-2xl border border-border/60 bg-background/60 p-4">
            ...
            <span className={cn(
              ...
              f.status === "Active" ? "bg-sage/20 text-foreground" : "bg-secondary text-muted-foreground",
            )}>
              {f.status}
            </span>
...
const NotificationsSection = () => {
  const groups: { title: string; items: { label: string; hint: string; defaultOn?: boolean }[] }[] = [
    {
      title: "Matches",
      items: [
        { label: "Daily curated set is ready", hint: "A gentle nudge each morning.", defaultOn: true },
        { label: "Mutual interest", hint: "When you and someone both express interest.", defaultOn: true },
      ],
    },
    {
      title: "Conversations",
      items: [
        { label: "New messages", hint: "Only from active conversations.", defaultOn: true },
        { label: "Expiring connections", hint: "A soft reminder before they close.", defaultOn: false },
      ],
    },
    {
      title: "Family activity",
      items: [
        { label: "Family member reviewed a profile", hint: "", defaultOn: true },
  ...
  return (
    <Surface>
      ...
      <div className="pb-4">
        {groups.map((g, gi) => (
          <div key={g.title}>
            <div className="px-7 pt-5 pb-2 text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{g.title}</div>
            {g.items.map((it) => (
  ...
  return (
    <Surface>
      ...
      <div className="px-7 pb-7 space-y-7">
        {/* Profile border */}
        <div>
          <div className="text-sm font-medium mb-3 flex items-center gap-2"><Crown className="w-4 h-4 text-accent" /> Profile border</div>
            ...
            {BORDERS.map((b) => (
              ...
              >
                <div className={cn(
                  ...
                  <img src={match1} alt="" className="w-full h-full rounded-full object-cover" />
                </div>
                <span className={cn(
                  "text-[11px]",
                  border === b.id ? "text-foreground font-medium" : "text-muted-foreground",
                )}>{b.label}</span>
              </button>
            ))}
          </div>
        ...
        {/* Mood line */}
        <div>
          <div className="text-sm font-medium mb-3 flex items-center gap-2"><Smile className="w-4 h-4 text-primary" /> Mood / status line</div>
          <Input
            value={mood}
        ...
        {/* Chat theme */}
        <div>
          <div className="text-sm font-medium mb-3 flex items-center gap-2"><Palette className="w-4 h-4 text-plum" /> Chat theme</div>
            ...
            {THEMES.map((t) => (
              <button
                ...
                className={cn(
                  ...
                  theme === t.id ? "border-primary/60 shadow-soft" : "border-border/60 hover:border-primary/30",
                )}
              >
        ...
        {/* Favorite song */}
        <div>
          <div className="text-sm font-medium mb-3 flex items-center gap-2"><Music2 className="w-4 h-4 text-primary-deep" /> Current favorite song</div>
          <div className="flex items-center gap-4 rounded-2xl border border-border/60 bg-gradient-blush p-4">
            <div className="relative w-14 h-14 rounded-xl bg-gradient-sunset shrink-0 grid place-items-center shadow-soft">
              ...
              {playing && <span className="absolute inset-0 rounded-xl ring-2 ring-primary/40 animate-pulse" />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-sm font-medium truncate">Tum Hi Ho</div>
              <div className="text-xs text-muted-foreground truncate">Arijit Singh · Aashiqui 2</div>
              <div className="mt-2 h-1 rounded-full bg-background/60 overflow-hidden">
                <div className={cn("h-full bg-gradient-sunset transition-all", playing ? "w-2/3" : "w-1/4")} />
              </div>
            </div>
            <button
...
const SafetySection = () => {
  const items = [
    { label: "Email", status: "verified", hint: "priya@example.com", icon: Mail },
    { label: "Phone number", status: "verified", hint: "+91 ••••• 4521", icon: Phone },
    { label: "Government ID", status: "pending", hint: "Submitted 2 days ago — usually reviewed within 48 hours.", icon: BadgeCheck },
    { label: "Photo authenticity", status: "unverified", hint: "A short live selfie helps families trust your profile.", icon: User },
  ];
  return (
    <Surface>
      <SectionHeader
      ...
      <div className="px-7 pb-7 space-y-3">
        {items.map((it) => {
          ...
          return (
            <div key={it.label} className="flex items-center gap-4 rounded-2xl border border-border/60 bg-background/60 p-4">
              ...
              <span className={cn("text-[10px] uppercase tracking-wider px-2.5 py-1 rounded-full", tone)}>
                {it.status}
              </span>
              {it.status !== "verified" && (
...
const ProfileSection = () => (
  <Surface>
    ...
    <div className="pb-4">
      ...
      <Divider />
      <Row title="Location" hint="Bengaluru, India" onClick={() => {}}>
        <ChevronRight className="w-4 h-4 text-muted-foreground" />
      </Row>
      <Divider />
      <Row title="Email" hint="priya@example.com">
        <span className="text-xs text-sage flex items-center gap-1"><BadgeCheck className="w-3.5 h-3.5" /> Verified</span>
      </Row>
      <Divider />
...
const Settings = () => {
  ...
  const renderSection = () => {
    switch (active) {
      ...
      case "photos": return <PhotoPrivacySection />;
    ...
    <div className="min-h-screen bg-gradient-blush">
      ...
      <header className="sticky top-0 z-30 backdrop-blur-xl bg-background/60 border-b border-border/60">
        <div className="container max-w-6xl flex items-center justify-between py-4">
          <Link to="/today" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
            <ArrowLeft className="w-4 h-4" /> Back
          </Link>
          <div className="text-center">
            <div className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">Your space</div>
            <h1 className="font-serif text-lg leading-tight">Settings & Privacy</h1>
          </div>
          <Link to="/matches" className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
      ...
      <div className="container max-w-6xl py-8 lg:py-12">
        ...
        <div className="mb-8 lg:mb-10 max-w-2xl animate-fade-in">
          <p className="text-[11px] uppercase tracking-[0.22em] text-primary-deep">Privacy-first by design</p>
          ...
          <p className="text-sm text-muted-foreground mt-3 leading-relaxed">
            Every setting here is built around respect — for you, your family, and the people you may meet. Adjust anything, anytime.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-[260px_1fr] gap-6 lg:gap-10">
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
                ...
                return (
                  ...
                  >
                    <div className={cn(
                      ...
                      isActive ? "bg-card text-primary-deep" : "bg-secondary/60 text-foreground",
                    )}>
                      <s.icon className="w-4 h-4" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className={cn("text-sm leading-tight", isActive ? "font-medium" : "")}>{s.label}</div>
            ...
            <div className="mt-5 rounded-3xl border border-border/60 bg-gradient-warm p-5 text-sm leading-relaxed shadow-soft">
              ...
              <p className="font-serif text-base leading-snug">Your privacy is sacred.</p>
              <p className="text-xs text-foreground/70 mt-2">
                We never share your details without your consent. Conversations and photos are protected by default.
              </p>
            </div>
          </aside>
